...
def print_log(s):
	with open('./UTD_single_scale_discuss.txt', 'a') as f:
		print(s, end="\n", file=f)
...
for _ in range(5):
	...
	for time in [2]:
		...
		for i in range(num_channels):
			...
			for j in range(4):
				conv = Conv2D(nb_filter, (nb_row[i], nb_col), kernel_initializer = kernel_initializer, activation = activation, padding = padding, strides = strides, data_format = data_format)(input)
