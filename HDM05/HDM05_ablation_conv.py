...
if not os.path.exists('./HDM05_esn_states.npy'):
	...
	cp.dump(echo_states, open('./HDM05_esn_states.npy', 'wb'))
else:
	echo_states = np.load('./HDM05_esn_states.npy')
...
for time in [2]:
	...
	for i in range(10):
		...
		for j in range(num_channels):
			...
			multi_pools.append(Dense(nb_filter, kernel_initializer = kernel_initializer, activation = activation)(Dropout(0.5)(input)))
		...
		echo_states_train = np.array(echo_states_train)  ## 
		echo_states_test = np.array(echo_states_test)

		
		
		echo_states_train = np.reshape(echo_states_train, (echo_states_train.shape[0], echo_states_train.shape[1],  echo_states_train.shape[-1] * echo_states_train.shape[-2] * echo_states_train.shape[-3]))
		echo_states_test = np.reshape(echo_states_test, (echo_states_test.shape[0], echo_states_test.shape[1], echo_states_test.shape[-1] * echo_states_test.shape[-2] * echo_states_test.shape[-3]))

		echo_states_train = [echo_states_train[i,:,:] for i in range(num_channels)]  ## 
		echo_states_test = [echo_states_test[i,:,:] for i in range(num_channels)]
